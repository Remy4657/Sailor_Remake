...
const userLoginFunc2 = async (req, res) => {
  try {
    //console.log('req body login: ', req.body)
    let data = await userService.userLogin2(req.body);
    if (data && data.EC === 1) {
      res.cookie("access_token", data.DT.access_token, { httpOnly: true });
      res.cookie("refresh_token", data.DT.refresh_token, { httpOnly: true });
    }
...
const refreshToken = async (req, res) => {
  try {
    ...
    if (data && +data.EC === 0) {
      // khong co refresh token hoac refresh_token het han
...
const loginByGoogle = async (req, res, next) => {
  try {
    ...
    let dataUserRegister = await userService.userGoogleRegister(
      req.userProfile
    );
